...
const Page = () => {
  ...
  const fetchWalletData = useCallback(async (walletAddress: string) => {
    try {
      ...
      // Add alternating curvature to each link
      graphData.links = graphData.links.map((link: Link, index: number) => ({
        ...link,
        curvature: (index - graphData.links.length/2) * GRAPH_CONFIG.LINK_CURVATURE
      }));
